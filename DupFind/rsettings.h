...
protected:
	...
	// DWORD 
	inline LONG SaveLoadItem(HKEY hKey, LPCTSTR szName, DWORD& Variable, bool bSave)
	{
	...
	// int
	inline LONG SaveLoadItem(HKEY hKey, LPCTSTR szName, int& Variable, bool bSave)
	{
		return SaveLoadItem(hKey, szName, (DWORD&)Variable, bSave);
	}

	// long
	inline LONG SaveLoadItem(HKEY hKey, LPCTSTR szName, long& Variable, bool bSave)
	{
		return SaveLoadItem(hKey, szName, (DWORD&)Variable, bSave);
	}

	// bool
	inline LONG SaveLoadItem(HKEY hKey, LPCTSTR szName, bool& Variable, bool bSave)
	{
		DWORD dwValue = Variable ? 1 : 0;
		LONG dwResult = SaveLoadItem(hKey, szName, dwValue, bSave);
	...
	// char
	inline LONG SaveLoadItem(HKEY hKey, LPCTSTR szName, char& Variable, bool bSave)
	{
	...
	// unsigned char
	inline LONG SaveLoadItem(HKEY hKey, LPCTSTR szName, unsigned char& Variable, bool bSave)
	{
		DWORD dwValue = (DWORD)Variable;
		LONG dwResult = SaveLoadItem(hKey, szName, dwValue, bSave);
		if(!bSave && dwResult == ERROR_SUCCESS)
	...
	inline LONG SaveLoadItem(HKEY hKey, LPCTSTR szName, wchar_t& Variable, bool bSave)
	{
		DWORD dwValue = (DWORD)Variable;
		LONG dwResult = SaveLoadItem(hKey, szName, dwValue, bSave);
		if(!bSave && dwResult == ERROR_SUCCESS)
	...
	// TCHAR*
	inline LONG SaveLoadItem(HKEY hKey, LPCTSTR szName, TCHAR* Variable, size_t size, bool bSave)
	{
		DWORD dwType = REG_SZ;
	...
	// void*
	inline LONG SaveLoadItem(HKEY hKey, LPCTSTR szName, void* Variable, size_t size, bool bSave)
	{
	...
	// std::string
	inline LONG SaveLoadItem(HKEY hKey, LPCTSTR szName, CRegSettings& Variable, bool bSave)
	{
		LONG dwResult = ERROR_SUCCESS;
		HKEY hKeySub;
		if(bSave)
	...
	inline LONG SaveLoadItem(HKEY hKey, LPCTSTR szName, CString& Variable, bool bSave)
	{
		if(bSave)
	...
	// std::string
	template <class T>
	inline LONG SaveLoadItemSTLstring(HKEY hKey, LPCTSTR szName, T& Variable, bool bSave)
	{
		if(bSave)
		{
	...
	LONG SaveLoadItemATL(HKEY hKey, LPCTSTR szName, TArray& Variable, bool bSave)
	{
		if(!bSave)
			Variable.RemoveAll();
		LONG dwResult = ERROR_SUCCESS;
...
ret:
	...
	LONG SaveLoadItemSTL(HKEY hKey, LPCTSTR szName, TSTLArray& Variable, bool bSave)
	{
		if(!bSave)
			Variable.clear();
		LONG dwResult = ERROR_SUCCESS;
...
public:
	LONG Save()
	{
		...
		dwResult = ::RegCreateKeyEx(m_hKey, m_sKey, NULL, NULL, REG_OPTION_NON_VOLATILE,
		...
		dwResult = InternalSaveLoad(hKey, true, false);
		::RegCloseKey(hKey);
		return dwResult;
	}
	...
	LONG Load()
	{
		...
		HKEY hKey;
		dwResult = ::RegOpenKeyEx(m_hKey, m_sKey, 0, KEY_READ, &hKey);
		...
		dwResult = InternalSaveLoad(hKey, false, false);
		::RegCloseKey(hKey);
		return dwResult;
	}
};
